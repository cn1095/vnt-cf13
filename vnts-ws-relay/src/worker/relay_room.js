...
export class RelayRoom {    
  ...
  // 新增：判断是否可以直接转发  
  shouldDirectForward(data) {  
    // 检查是否为标准数据包，无需复杂解析  
    if (data.length >= 12) {  
      const protocol = data[1];  
      const transport = data[2];  
      // 直接转发常见的数据包类型  
      return protocol === 4 && transport === 4; // IPTURN 数据包  
    }  
    return false;  
  }  
  
  // 新增：直接转发方法  
  async directForward(clientId, data) {  
    console.log(`[DEBUG] Direct forwarding packet from ${clientId}`);  
    ...
    for (const [targetClientId, server] of this.connections) {  
      ...
      try {  
        ...
        console.error(`Direct forward to ${targetClientId} failed:`, error);  
      }  
    }  
  }  
  ...
  async handleMessage(clientId, data) {    
    try {    
      ...
      // 快速路径：直接转发，减少解析开销  
      if (this.shouldDirectForward(uint8Data)) {  
        return await this.directForward(clientId, uint8Data);  
      }  
      ...
      if (this.shouldBroadcast(packet)) {    
        await this.optimizedBroadcast(clientId, packet.buffer());    
      }    
  ...
  // 新增：优化的广播方法  
  async optimizedBroadcast(senderId, rawBuffer) {  
    console.log(`[DEBUG] Optimized broadcasting from ${senderId}`);  
    ...
    for (const [clientId, server] of this.connections) {  
      ...
      try {  
        // 直接发送原始缓冲区，避免复制和重新解析  
        server.send(rawBuffer);  
      } catch (error) {  
        console.error(`Optimized broadcast to ${clientId} failed:`, error);  
      }  
    }  
  }  
