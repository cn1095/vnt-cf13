...
export class RelayRoom {    
  ...
  parseVNTHeader(buffer) {  
    ...
    return {  
        source: (buffer[4] << 24) | (buffer[5] << 16) | (buffer[6] << 8) | buffer[7],  
        destination: (buffer[8] << 24) | (buffer[9] << 16) | (buffer[10] << 8) | buffer[11],  
        protocol: buffer[1],  
  ...
  async handleMessage(clientId, data) {  
    try {  
      ...
      console.log(`[调试] 数据十六进制: ${Array.from(uint8Data).map(b => b.toString(16).padStart(2, '0')).join('')}`);  
      ...
      console.log(`[DEBUG] Parsed VNT header: protocol=${header.protocol}, transport=${header.transportProtocol}, source=${header.source}, dest=${header.destination}`);  
  ...
  async fullParsingPath(clientId, data) {  
    ...
    console.log(`[DEBUG] Packet protocol: ${packet.protocol}, transport: ${packet.transportProtocol}`);  
      
    const response = await this.packetHandler.handle(context, packet, addr, clientId);  
  ...
  async headerBasedForward(clientId, data, header) {  
    ...
    for (const [targetClientId, server] of this.connections) {  
      ...
      try {  
        ...
        console.error(`Header-based forward to ${targetClientId} failed:`, error);  
      }  
    }  
  }  
